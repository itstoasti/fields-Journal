...
export function buildGrokPrompt(data: PromptInterpolationData): string {
  const kw1 = (data.keywords[0] || 'Horizon').trim();
  const kw2 = (data.keywords[1] || 'Silence').trim();
  const kw3 = (data.keywords[2] || 'Passage').trim();
  const place = (data.place || 'Field Observation').trim();
  const number = (data.number || '01').trim();
  const year = (data.year || new Date().getFullYear().toString()).trim();

  return LOCKED_GROK_PROMPT_TEMPLATE
    .replace('{{PLACE}}', place)
    .replace('{{NUMBER}}', number)
    .replace('{{KEYWORD_1}}', kw1)
    .replace('{{KEYWORD_2}}', kw2)
    .replace('{{KEYWORD_3}}', kw3)
    .replace('{{YEAR}}', year);
}
